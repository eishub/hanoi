...
public class Hanoi {
	 ...
	 * 
	 * @param list
	 *            list with parameters to set up the game.
	 */
	...
	/**
	 * Resets the game to the provided setup. If no list is supplied, default
	 * values will be used.
	 * 
	 * @param list
	 *            list with parameters to set up the game.
	...
	public void reset(List<Integer> list) {
		...
		for (Integer x : list) {
			if (x >= 0 && x <= 2)
				...
				System.err
						.println("[ERROR] [RESET] Error in parameters for setting up the game. -> "
								+ x);
		}
	 ...
	 *
	 * @param args
	 *            arguments to be supplied to Hanoi.
	 */
	 ...
	 *
	 * @param origin
	 *            Pin to get the disc from.
	 * @param destination
	 *            Pin to move the disc to. TODO: This can be done in a cleaner
	 *            way
	 */
	...
	public void possibleMoves() {
		...
		System.out.println((top[0] < top[1] ? "[L -> C]" : "[      ]") + " "
				+ (top[0] < top[2] ? "[L -> R]" : "[      ]") + " "
				+ (top[1] < top[0] ? "[C -> L]" : "[      ]") + " "
				+ (top[1] < top[1] ? "[C -> R]" : "[      ]") + " "
				+ (top[2] < top[0] ? "[R -> L]" : "[      ]") + " "
	 ...
	 *
	 * @param p1
	 *            int identifying the origin pin.
	 * @param p2
	 *            int identifying the destination pin.
		...
		if (p1 >= 0 && p1 <= 2 && p2 >= 0 && p2 <= 2) {
			if (!pins[p1].isEmpty()) {
				if (pins[p2].isEmpty()) {
					...
					return pins[p1].getTopDisc().getSize() < pins[p2]
							.getTopDisc().getSize();
				}
			}
		}
