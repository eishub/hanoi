...
public class HanoiUI extends JFrame {
    ...
    public void drawPins(Graphics g) {
        ...
        g.fillRect(PIN_L_X, WINDOW_HEIGHT - (SPACER + PIN_BASE_HEIGHT), PIN_BASE_WIDTH, PIN_BASE_HEIGHT);
        g.fillRect(PIN_M_X, WINDOW_HEIGHT - (SPACER + PIN_BASE_HEIGHT), PIN_BASE_WIDTH, PIN_BASE_HEIGHT);
        g.fillRect(PIN_R_X, WINDOW_HEIGHT - (SPACER + PIN_BASE_HEIGHT), PIN_BASE_WIDTH, PIN_BASE_HEIGHT);

        // Draw the poles of the Pins.
        // TODO: Fix this dirty hack at the end? (+SPACER)
        g.fillRect(PIN_L_X + PIN_CENTER, WINDOW_HEIGHT - (SPACER + PIN_BASE_HEIGHT), PIN_WIDTH, -PIN_HEIGHT + SPACER);
        g.fillRect(PIN_M_X + PIN_CENTER, WINDOW_HEIGHT - (SPACER + PIN_BASE_HEIGHT), PIN_WIDTH, -PIN_HEIGHT + SPACER);
        g.fillRect(PIN_R_X + PIN_CENTER, WINDOW_HEIGHT - (SPACER + PIN_BASE_HEIGHT), PIN_WIDTH, -PIN_HEIGHT + SPACER);
    }

    /**
     * Draws the discs.
     *
     * @param g    Graphics object form the paint function.
     * @param pins all Pins collected in an array.
     */
    public void drawDiscs(Graphics g, Pin[] pins) {
        ...
        for (Pin p : pins) {
            Disc tmp = p.discs;
            int count = 1; // Keep track of the amount of discs processed on this pin.

            // Loop through all discs on this pin (LinkedList principle).
            while (tmp != null) {
                // Determine drawing positions.
                int w = 20 + tmp.getSize() * DISC_DELTA_WIDTH;
                int x = SPACER + (p.id * (PIN_BASE_WIDTH + SPACER)) + (PIN_CENTER - (w / 2) + (PIN_WIDTH / 2));
                int y = WINDOW_HEIGHT - SPACER - PIN_BASE_HEIGHT - (count * DISC_HEIGHT);
    ...
    /**
     * Forwards a call to the move function to the move function in the Game itself.
     *
     * @param origin      Pin object to take a Disc from, this Pin can not be empty!
     * @param destination Pin to push the disc onto.
     */
